...
空间复杂度是O(1)，所以排除map。也不能先排序，因为时间复杂度不允许。那么就可以借助数组下标，例如`a[1] = 5` 那么我们就把这个 5 移动到 a[4] 里面去，如果 a[4] 里面的值已经是 5 了，那么就改放一个占位符 例如Integer.MAX_VALUE之类的。一般来说是ok了，本题，可能会存在数组下标越界的问题，`a[i] = 10000` 直接超过了数组长度。那么我的做法是
