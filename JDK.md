...
4. 容量是哈希表中桶的数量，初始容量只是哈希表在创建时的容量，**默认大小为16**，选的需要是 $2^n$的形式。1是方便做key的与运行|2是在方便扩容后元素的移动 。

5. 加载因子是哈希表在其容量自动增加之前可以达到多满的一种尺度，**默认为 0.75f**，这个值是大量统计后得出。
...
<img src="截图/JDK/concurrentHashMap扩容.png" alt="img" style="zoom:80%;" />
...
在java虚拟机规范中，要求实现4种内存屏障（LoadLoadBarrier、LoadStoreBarrier、SL、SS），LoadLoadBarrier 的意思就是在连续两个读指令的情况下，不允许换位置。
...
<u>由于内存写操作同时会无效化其他处理器所持有的、指向同一内存地址的缓存行</u>，因此可以认为其他处理器能够立即见到该 volatile 字段的最新值。
...
3. 最后调用astore_1 命令建立内存与对象之间的关系 `7 astore_1`

由于这是很多条指令，jvm可能会乱序执行。这个时候如果线程B在执行if (ins == null)时，正常情况下，如果为true，说明需要获取Instance.class锁，等待初始化。但是这时候，假设线程A再没有对ins进行初始化完，只进行了半初始化，比如只对成员变量str进行了赋值，还没有来的及对成员变量a进行赋值，假如jvm将未完成赋值的值拷贝回堆中，这个时候线程B有可能读到的值就不是为null了，就会造成数据丢失的情况。这时候我们发现线程B获取的对象中a的值是0，而不是20。
...
1.如果为**可偏向状态**, 则尝试用 CAS 操作， 将自己的线程 ID 写入MarkWord
...
如果 CAS 操作失败， 则说明， 有另外一个线程 Thread B 抢先获取了偏向锁。 这种状态说明该对象的竞争比较激烈， 此时需要**撤销** Thread B 获得的偏向锁，将 Thread B 持有的锁升级为轻量级锁。 该操作需要等待全局安全点 JVM safepoint ( 此时间点， 没有线程在执行字节码)。

2.如果是**已偏向状态**， 则检测 MarkWord 中存储的 thread ID 是否等于当前 thread ID 。
...
一种是**不可偏向的无锁状态**， 如下图（之所以不允许偏向， 是因为已经检测到了多于一个线程的竞争， 升级到了轻量级锁的机制）

另一种是**不可偏向的已锁 ( 轻量级锁) 状态**

之所以会出现上述两种状态， 是因为偏向锁**不存在解锁的操作**， 只有**撤销**操作。 触发**撤销**操作时：

- 原来已经获取了偏向锁的线程可能已经执行完了同步代码块， 使得对象处于 “**闲置状态”**，相当于原有的偏向锁已经过期无效了。此时该对象就应该被直接转换为**不可偏向的无锁状态**。
- 原来已经获取了偏向锁的线程也可能尚未执行完同步代码块， 偏向锁依旧有效， 此时**对象就应该被转换为被轻量级加锁的状态**
...
- - 首先根据标志位判断出对象状态处于不可偏向的无锁状态( 如下图）
     ![这里写图片描述](截图/JDK/轻量级锁.png)
  - 在当前线程的**栈桢**（Stack Frame）中创建用于存储**锁记录**（lock record）的空间，并将对象头中的Mark Word复制到锁记录中，官方称为Displaced Mark Word。
...
2. 多个线程同时执行同一段同步代码时，ObjectWaiter先进入\_EntryList队列，当某个线程获取到对象的monitor以后进入_Owner区域，并把monitor中的owner变量设置为当前线程同时monitor中的计数器count+1；
...
https://baijiahao.baidu.com/s?id=1683595351167222334
...
[偏向锁，轻量级锁，重量级锁的加锁过程](https://www.cnblogs.com/zhouxiaohong/p/13654419.html)
...
3. 所谓的获取锁其实是用一个CAS操作，判断state是否是0（表示当前锁未被占用），如果是0则把它置为1，并且设置当前线程为该锁的独占线程，表示获取锁成功。
4. “当前线程”尝试失败的情况下，先通过addWaiter(Node.EXCLUSIVE)来将“当前线程”加入到"CLH队列(非阻塞的FIFO队列)"末尾。CLH队列就是线程等待队列。
5. 再执行完addWaiter(Node.EXCLUSIVE)之后，会调用acquireQueued()来获取锁。由于此时ReentrantLock是公平锁，它会**根据公平性原则**来获取锁。
6. “当前线程”在执行acquireQueued()时，会进入到CLH队列中休眠等待，直到获取锁了才返回！如果“当前线程”在休眠等待过程中被中断过，acquireQueued会返回true，此时"当前线程"会调用selfInterrupt()来自己给自己产生一个中断。
...
static final class FairSync extends Sync {
    ...
    protected final boolean tryAcquire(int acquires) {
        ...
        int c = getState();
        if (c == 0) {
          	// 在加锁的时候需要先去判断自己是不是等待队列的队头，是的话才去尝试CAS
...
哪个在前面调用并没有关系，可以先让一个线程unpark，unpark就是给一个线程打一个标记，虽然当前还没有堵塞，当线程需要堵塞的时候，发现已经有这个标记了，那么就直接不用堵塞了，直接指向。// todo
...
首先wait与notify都是只能在获取了锁的情况下使用，需要配合Synchronized。

`lock.wait` 的意思是：持有了这把锁的这个线程，进入这把锁的等待队列，同时释放这把锁。
...
自动装箱实际上是一种语法糖。它们发生在**编译阶段**，生成的字节码是一致的。javac替我们自动把装箱转换为Integer.valueOf()，把拆箱替换为Integer.intValue() 
...
Integer等包装类，定义了类似SIZE或者BYTES这样的常量，这反映了什么样的设计考虑呢？如果你使用过其他语言，比如C、C++，类似整数的位数，其实是不确定的，可能在不同的平台，比如32位或者64位平台，存在非常大的不同。那么，在32位JDK或者64位JDK里，数据位数会有不同吗？或者说，这个问题可以扩展为，我使用32位JDK开发编译的程序，运行在64位JDK上，需要做什么特别的移植工作吗？ 其实，这种移植对于Java来说相对要简单些，因为原始数据类型是不存在差异的，这些明确定义在Java语言规范里面，不管是32位还是64位环境，开发者无需担心数据的位数差异。
