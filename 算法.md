...
每轮都确定一个绝对位置上的数字。通过遍历右边的所有数字，得出这个数字。
    ...
    private void sink(int k){
        while(k * 2 <= N){
            int j = k * 2;
          	...
          	// 这里注意这个小于号，没有等于号了，因为可能最后一个节点只有左孩子 如果j等于左孩子了 
          	// 那也没有必要去比较两个孩子的大小了
            if(j < N && less(j, j + 1)){
                j++;
          	}
...
这里有各种背包问题的总结： https://app.gitbook.com/@maoshifan1989/s/python/hui-su-yu-dong-tai-gui-hua/bei-bao-dp
...
[[宫水三叶] 分享｜深度讲解背包问题：面试中每五道动态规划就有一道是背包模型 ...](https://leetcode-cn.com/circle/discuss/GWpXCM/)
...
借助位运算实现。如IP10.0.3.193，将10左移24位，0左移16位，3左移8位，193左移0位。4个seg`或运算`，即为结果。这里进行或运算是最好的，当然也可以直接进行加法运算，最后的结果也是对的。

如果使用java语言需要注意int类型的溢出问题，当最高位为1的时候，其实就已经溢出了。
...
//        String ip = "10.0.3.193";
        ...
        System.out.println((int)ip2int(ip));
        ...
        long seg0 = (Long.parseLong(split[0]) << 24);
        long seg1 = (Long.parseLong(split[1]) << 16);
        long seg2 = (Long.parseLong(split[2]) << 8);
        long seg3 = Long.parseLong(split[3]);
        return seg0 | seg1 | seg2 | seg3;
    }

    private static String int2ip(int value){
        int seg4 = value & 255;
        int seg3 = (value >> 8) & 255;
        int seg2 = (value >> 16) & 255;
        int seg1 = (value >> 24) & 255;
...
> 作者：一个搬砖的胖子
> 链接：https://leetcode-cn.com/circle/discuss/l8fl8B/
> 来源：力扣（LeetCode）
> 著作权归作者所有。商业转载请联系作者获得授权，非商业转载请注明出处。
